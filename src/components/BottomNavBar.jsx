...
const BottomNavBar = ({ activeTab, setActiveTab, backgroundColor = '#212124' }) => {
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 text-white`} style={{ backgroundColor, borderTop: backgroundColor === '#212124' ? '1px solid #424245' : 'none' }}>
      <ul className="flex justify-around items-end h-20 relative"> {/* Increased height from h-16 to h-20 */}
        {navItems.map((item, index) => (
          <li key={item.id} className={cn(
            "flex-1",
            item.id === 'walk' ? "absolute left-1/2 transform -translate-x-1/2 -top-10" : "", // Adjusted top position
            index === 1 ? "mr-8" : "",
            index === 3 ? "ml-8" : ""
          )}>
            <button
              ...
              className={cn(
                ...
                item.id === 'walk'
                  ? "bg-blue-500 rounded-full w-20 h-20 shadow-lg" // Increased size from w-16 h-16 to w-20 h-20
                  : "pt-2",
                activeTab === item.id ? "text-white" : "text-gray-400"
              )}
            >
              {item.id === 'profile' ? (
                <Avatar className="h-8 w-8"> {/* Increased size from h-6 w-6 to h-8 w-8 */}
                ...
                <item.icon className={cn("h-8 w-8 stroke-1", item.id === 'walk' ? "stroke-2" : "")} /> // Increased size from h-6 w-6 to h-8 w-8
              )}
              {item.id !== 'walk' && (
                <span className="text-sm mt-1">{item.label}</span> // Increased text size from text-xs to text-sm
              )}
            </button>
          </li>
        ))}
